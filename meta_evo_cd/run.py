...
def summarize_task_batch(tasks) -> dict:
    """
    Summarize the sampled task batch so logs can be interpreted.
    Works even if some tasks are linear (nonlinear_spec=None).
    """
    families = []
    for t in tasks:
        if getattr(t, "nonlinear_spec", None) is not None:
            families.append(t.nonlinear_spec.family)
        else:
...
def evaluate_cfg_on_tasks(cfg: PipelineConfig, tasks) -> Individual:
    ...
    for t in tasks:
        ...
        X = simulate_sem(
            A_true, t.n, t.sem_type, t.noise_type, t.noise_scale, rng, 
...
def main():
    ...
    header = [
        ...
        "skeleton","alpha","max_cond_set","corr_thresh","max_edges", "orient",
        "meek_passes", "prune_marginal_alpha"
    ]
    ...
    for gen in range(args.generations):
        ...
        for rank, ind in enumerate(sel_sorted[: min(len(sel_sorted), 10)]):
            ...
            append_jsonl(jsonl_path, {**row, "task_family_counts": task_family_counts})
            append_csv(csv_path, row, header=header + FAM_COLS)
